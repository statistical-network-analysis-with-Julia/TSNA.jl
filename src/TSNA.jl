...
using NetworkDynamic: spell_active_at, elapsed_seconds
using SNA
using Statistics
...
function earliest_arrival(dnet::DynamicNetwork{T, Time}, source::T, start_time;
                          ...
                          target::Union{Nothing, T}=nothing,
...
function earliest_arrival!(ws::TemporalPathWorkspace{T, Time},
    ...
    1 <= source <= nv(dnet) || throw(ArgumentError("source must be a vertex of the network"))
    isnothing(target) || 1 <= target <= nv(dnet) ||
        throw(ArgumentError("target must be a vertex of the network"))
...
function earliest_arrival_all(dnet::DynamicNetwork{T, Time}, start_time;
    ...
    for s in sources
        arrival, _ = earliest_arrival!(ws, dnet, T(s), start_time; end_time=end_time, missing=missing)
...
function temporal_distance(dnet::DynamicNetwork{T, Time}, source::T, target::T,
                           start_time; end_time=dnet.observation_period[2],
                           missing::Symbol=:error) where {T, Time}
...
function backward_reachable_set(dnet::DynamicNetwork{T, Time}, target::T, end_time;
                                start_time=dnet.observation_period[1],
                                missing::Symbol=:error) where {T, Time}
    require_observed(dnet.network, missing; context="backward_reachable_set")
    1 <= target <= nv(dnet) || throw(ArgumentError("target must be a vertex of the network"))
    start_time <= end_time || throw(ArgumentError("start_time must be <= end_time"))
...
function _snapshot(dnet, at; missing::Symbol=:error, active_only::Bool=false)
...
function t_degree(dnet::DynamicNetwork{T, Time}, at; mode::Symbol=:total,
    ...
    return SNA.degree_centrality(_snapshot(dnet, at; missing); mode, missing, kwargs...)
end
...
"""
t_betweenness(dnet::DynamicNetwork, at; normalized::Bool=false, missing::Symbol=:error,
              kwargs...) =
    SNA.betweenness_centrality(_snapshot(dnet, at; missing); normalized, missing, kwargs...)
...
`tCloseness`.
"""
t_closeness(dnet::DynamicNetwork, at; missing::Symbol=:error, kwargs...) =
    SNA.closeness_centrality(_snapshot(dnet, at; missing); missing, kwargs...)
...
alias `tEigenvector`.
"""
t_eigenvector(dnet::DynamicNetwork, at; missing::Symbol=:error, kwargs...) =
    SNA.eigenvector_centrality(_snapshot(dnet, at; missing); missing, kwargs...)
...
"""
t_pagerank(dnet::DynamicNetwork, at; damping::Float64=0.85, missing::Symbol=:error) =
    SNA.pagerank(_snapshot(dnet, at; missing); α=damping, missing)
...
"""
    t_density(dnet::DynamicNetwork, at; active_only=true, missing=:error) -> Float64

Density at time `at`, over active vertices by default. Pass `active_only=false`
to use the complete vertex universe. The missing-data policy is checked on
the original network before inactive vertices are removed.
...
function t_density(dnet::DynamicNetwork, at; active_only::Bool=true,
    ...
    return SNA.density(_snapshot(dnet, at; missing, active_only); missing, diag)
end
...
"""
    t_reciprocity(dnet::DynamicNetwork, at; method=:dyadic, missing=:error) -> Float64

Dyadic reciprocity at time `at`, following SNA's `method=:dyadic` convention.
Pass `method=:edgewise` for the fraction of edges that are reciprocated.
...
function t_reciprocity(dnet::DynamicNetwork, at; method::Symbol=:dyadic,
    ...
    return SNA.reciprocity(_snapshot(dnet, at; missing, active_only); method, missing)
end
...
"""
t_transitivity(dnet::DynamicNetwork, at; missing::Symbol=:error, kwargs...) =
    SNA.transitivity(_snapshot(dnet, at; missing); missing, kwargs...)
...
function t_edge_duration(dnet::DynamicNetwork{T, Time};
                         mode::Symbol=:spell, aggregate::Symbol=:mean,
                         missing::Symbol=:error) where {T, Time}
...
function t_turnover(dnet::DynamicNetwork{T, Time}, window_size;
    ...
    t = start_time
    while t < end_time
        w_end = min(_next_window(t, window_size), end_time)
...
function tie_decay(dnet::DynamicNetwork{T, Time};
                   ...
                   at=dnet.observation_period[2], missing::Symbol=:error) where {T, Time}
...
function t_sna_stats(dnet::DynamicNetwork{T, Time}, times::AbstractVector;
    ...
    for t in times
        ...
        for m in measures
            row[m] = if m == :density
                SNA.density(snapshot; missing, diag=false)
            elseif m == :reciprocity
                SNA.reciprocity(snapshot; method=reciprocity_method, missing)
            elseif m == :transitivity
                SNA.transitivity(snapshot; missing)
...
function _check_window(start_time, end_time, window_size)
