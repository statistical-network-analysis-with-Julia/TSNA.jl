...
- **Breaking:** statistical and temporal-path queries reject unobserved dyads
  by default. Explicit `missing=:face` uses the recorded edges/spells and is
  forwarded to SNA. Mask-preserving `t_aggregate` remains a conversion.
- **Breaking:** `t_density` and `t_sna_stats` use active vertices by default;
  `active_only=false` retains the whole vertex universe. `t_reciprocity` uses
  dyadic reciprocity, including null dyads; use `method=:edgewise` for the old
  statistic. All delegate to SNA's shared definitions.
- Date/DateTime durations, decay, turnover and weighted aggregation use the
  public `NetworkDynamic.elapsed_seconds` helper. All-pairs distances store
  duration values rather than timestamps.
- Zero, negative, non-advancing windows and infinite observation windows now
  fail explicitly instead of looping indefinitely. Path source/target and
  time-order validation gives actionable errors.
- Added CI scaling gates for temporal path searches at fixed spell density.
- Fixed the path guide's illustrative type definition, which shadowed the
  real `TemporalPath`; repaired the benchmark dependency path.
