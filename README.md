...
Use `missing=:face` only when analysis of recorded face values is intended.
Density and `t_sna_stats` count active vertices by default; pass
`active_only=false` for the complete vertex universe. Reciprocity is dyadic
(including null dyads); `method=:edgewise` selects the edgewise statistic.
Per-vertex centrality vectors keep the original vertex indexing.

For Date/DateTime networks, durations and rates use seconds; numeric time
axes keep their native units. Window sizes must advance the time axis.
