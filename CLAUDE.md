...
Per-vertex measures retain all vertices and preserve original indexing.
Scalar density/reciprocity and `t_sna_stats` use active vertices by default
(`active_only=false` opts into the whole universe). Delegates are qualified
...
`NetworkDynamic.spell_active_at` and `elapsed_seconds` are imported public
...
Local dependencies Networks, NetworkDynamic and SNA use `[sources]` paths to
sibling directories (`../Networks.jl`, `../NetworkDynamic.jl`, `../SNA.jl`).
