...
Path queries reject masked dyads by default. Pass `missing=:face` explicitly
to traverse their recorded spells. This policy is checked before consulting
the cached contact index, so adding a mask cannot reuse an earlier observed
answer silently. Sources and targets must be valid vertex IDs.
